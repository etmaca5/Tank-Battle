...
void main_menu {
    SDL_Window *window = SDL_CreateWindow("Main Menu", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 
                                           MAX_WIDTH_GAME, MAX_HEIGHT_GAME, 0);
    SDL_Renderer *renderer = SDL_CreateRenderer(window, -1, 0);
    SDL_Event event;
    bool running = true;

    button_t *start_button = malloc(sizeof(button_t));
    start_button->rectangle = {(int)MAX_WIDTH_GAME / 4, (int)(MAX_HEIGHT_GAME - MAX_HEIGHT_GAME / 6), START_BUTTON_WIDTH, START_BUTTON_HEIGHT};
    start_button->id = START_BUTTON_ID;
    start_button->text = "Start Game";

    button_t *options_button = malloc(sizeof(button_t));
    options_button->rectangle = {(int)MAX_WIDTH_GAME / 10, (int)MAX_HEIGHT_GAME / 3, OPTIONS_BUTTON_WIDTH, OPTIONS_BUTTON_HEIGHT};
    options_button->id = START_BUTTON_ID;
    options_button->text = "Options";

    button_t *exit_button = malloc(sizeof(button_t));
    options_button->rectangle = {(int) 3 * MAX_WIDTH_GAME / 5, (int) MAX_HEIGHT_GAME / 3, EXIT_BUTTON_WIDTH, EXIT_BUTTON_HEIGHT};
    options_button->id = START_BUTTON_ID;
