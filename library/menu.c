...
list_t *make_button(vector_t corner, double width, double height) {
...
void menu_handler(char key, key_event_type_t type, double held_time, state_t *state, vector_t loc) {
  switch(key) {
    case MOUSE_CLICK: {
      ...
      vector_t diff = vec_subtract(loc, start_loc);
      double magnitude = sqrt(diff.x * diff.x + diff.y * diff.y);
      if (magnitude <= START_BUTTON_RADIUS) {
        //handle if it's inside the rectangles
        sdl_clear();
      }
      break;
    }
...
void menu_init() {  
  ...
  list_t *start_button = make_circle(start_loc, START_BUTTON_RADIUS);
  rgb_color_t green = {0, 1, 0};
  sdl_draw_polygon(start_button, green);

  list_t *options_button = make_button(tank_options_loc, 160.0, 100.0);
  rgb_color_t light_gray = {0.86, 0.86, 0.86};
  sdl_draw_polygon(options_button, light_gray);

  list_t *exit_button = make_button(exit_loc, 160.0, 100.0);
